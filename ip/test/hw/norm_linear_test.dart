// LoomNormLinear: the nonlinear->linear sequencer brick. An on-chip FSM runs
// two-pass RMSNorm over x, pipes the normalized vector straight into the matmul
// engine, and fires it: y = W @ rmsNorm(x, gamma). Matches the golden within
// fp16 + W4A8 tolerance, proving the op-to-op handoff the transformer sequencer
// performs at every projection.
...
import 'package:loom/src/hw/norm_linear.dart';
...
void main() {
  ...
  test('sequencer computes y = W @ rmsNorm(x, gamma) to golden', () async {
    const rows = 4, cols = 6;
    const peR = 2, peC = 2;
    const eps = 0.01;
    ...
    final gamma = Float64List.fromList([
      for (var c = 0; c < cols; c++) 0.5 + (c % 3) * 0.5,
    ]);

    final normed = rmsNorm(x, gamma, eps);
    final goldenFp = linear(w, rows, cols, normed);
    ...
    final dut = LoomNormLinear(maxColTiles: 4, maxRowBlocks: 4);
    for (final (p, s) in [
      ('clk', clk),
      ('reset', reset),
      ('start', start),
    ...
    // LOAD: stream x + gamma together, then per-row weight scales.
    for (var c = 0; c < cols; c++) {
      xEn.inject(1);
      xInL.inject(e(x[c]));
      gInL.inject(e(gamma[c]));
      await clk.nextPosedge;
    }
    xEn.inject(0);
    for (var r = 0; r < rows; r++) {
    ...
    for (var r = 0; r < rows; r++) {
      expect(
        got[r],
        closeTo(goldenFp[r], 0.2 + goldenFp[r].abs() * 0.25),
